...
def scan(
    directory: Path = typer.Argument(
        ..., help="Directory containing infrastructure files"
    ),
):
    ...
    nodes, edges = parse_all(directory)
    console.print(
        f"Found [bold blue]{len(nodes)}[/bold blue] resources and [bold blue]{len(edges)}[/bold blue] dependencies."
    )
...
def impact(
    ...
    resource: str = typer.Option(
        ..., "--resource", "-r", help="Resource ID to analyze impact for"
    ),
):
    ...
    if not impacted:
        console.print(
            f"No impact found or resource [bold red]{resource}[/bold red] does not exist/has no dependencies."
        )
        return
...
def export(
    ...
    format: str = typer.Option(
        "json", "--format", "-f", help="Export format (json, graphml, neo4j)"
    ),
    output: str = typer.Option(
        "graph_output", "--output", "-o", help="Output file path"
    ),
):
    ...
    try:
        ...
        console.print(
            f"Successfully exported graph to [bold green]{output}[/bold green] using [bold blue]{format}[/bold blue] format."
        )
